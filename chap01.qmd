...
namely, they satisfy:
$$
    \bomega^i(\mathbf{e}_j)=\delta^i_j.
$$
...
$$
    \mathbf{\alpha}=\alpha_{\red \alpha}\bomega^{\red \alpha}.
$$ {#eq-EinsiteinSumCo}
...
$$
    \bv\otimes \bw(\mathbf{\alpha}, \mathbf{\beta})=\bv(\mathbf{\alpha})\bw(\mathbf{\beta})\in \RR.
$$
elements in $V\otimes W$ are called **tensors**. Let $\{\mathbf{e}_i\}_{1\le i\le n}$ (resp. $\{\bff_j\}_{1\le j\le m}$) be a basis of $V$ (resp. $W$), then $V\otimes W$ has a basis 
$$
    \{\mathbf{e}_i\otimes \bff_j\}_{1\le i\le n, 1\le j\le m}.
$$
...
2) Let $\bT: V \to V$ be a linear transformation, then $\bT$ can be viewed as a $(1, 1)$ tensor. In fact let $\{\mathbf{e}_i\}_{1\le i\le n}$ be a basis of $V$ and $\{\bomega^j\}_{1\le j\le n}$ be the dual basis of $V^*$. Let $(a_i^j)_{1\le i, j\le n}$ be the coefficients defined by 
$$
    \bT(\mathbf{e}_i)=\mathbf{e}_{\alpha} a_i^{\alpha},
$$
for $i=1, \cdots, n$. Therefore
$$
    \bT=a_i^\alpha \mathbf{e}_{\alpha}\otimes \bomega^i.
$$

To calculate $\bT(v)$ for $\bv=\mathbf{e}_{\alpha}v^{\alpha}$:
$$
    \bT(\bv)=a_i^{\beta} \delta_{\alpha}^i \mathbf{e}_{\beta} \cdot v^{\alpha} = \mathbf{e}_{\beta}(a_{\alpha}^{\beta} v^{\alpha}) ,
$$
the right hand side is matrices multiplication.

3) Let $\Phi: V\times V \to \RR$ be a bilinear form, it can be viewed as a $(0, 2)$-tensor. Let $\{\mathbf{e}_i\}_{1\le i\le n}$ be as above. We can define
$$
...
$$
    \bT=T^{j_1, \cdots, j_r}_{i_1, \cdots, i_s}\mathbf{e}_{j_1}\otimes\cdots \otimes \mathbf{e}_{j_r}\otimes \bomega^{i_1}\otimes\cdots \otimes \bomega^{i_s},
$$
where the coefficient is given by:
$$
    \bT^{j_1, \cdots, j_r}_{i_1, \cdots, i_s}:=T(\bomega^{j_1},\cdots, \bomega^{j_r}, \mathbf{e}_{i_1}, \cdots, \mathbf{e}_{i_s}).
$$
...
$$
\begin{aligned}
    &\Cont^k_l(\mathbf{e}_{j_1}\otimes\cdots \otimes \mathbf{e}_{j_r}\otimes \bomega^{i_1}\otimes\cdots \otimes \bomega^{i_s})\\
    &=\bomega^{i_l}(\mathbf{e}_{j_k})\mathbf{e}_{j_1}\otimes\cdots \otimes \hat{\be}_{j_k}\otimes\cdots  \otimes\mathbf{e}_{j_r}\otimes \bomega^{i_1}\otimes\cdots\otimes \hat{\bomega}^{i_l} \otimes\cdots\otimes \bomega^{i_s}\\
    &=\delta_{j_k}^{i_l}\mathbf{e}_{j_1}\otimes\cdots \otimes \hat{\be}_{j_k}\otimes\cdots  \otimes\mathbf{e}_{j_r}\otimes \bomega^{i_1}\otimes\cdots\otimes \hat{\bomega}^{i_l} \otimes\cdots\otimes \bomega^{i_s},
\end{aligned}
$$
...
$$
    \bT=T^{\beta}_{\alpha}\mathbf{e}_{\beta}\otimes \be^{\alpha}.
$$
...
::: {.callout-important icon="false"}
## Remark
Let $\bT$ be a linear map from $V$ to $V$, then $\bT$ is a $(1, 1)$-tensor. Its dual map $\bT^*: V^*\to V^*$ is also a $(1, 1)$ tensor. What's the difference between these two tensors? Let $\{\mathbf{e}_1, \cdots, \mathbf{e}_n\}$ be a basis of $V$ and  $\{\bomega^1, \cdots, \bomega^n\}$ be the dual basis. We write
$$
    \bT=T^{\alpha}_{\beta} \mathbf{e}_{\alpha}\otimes \bomega^{\beta}.
$$
...
$$
    \bT^*=T^{\beta}_{\alpha} \bomega^{\alpha}\otimes \mathbf{e}_{\beta},
$$
Note that we intensionally swap the order of $\mathbf{e}_{\alpha}\otimes \bomega^{\beta}$ to $\bomega^{\alpha}\otimes \mathbf{e}_{\beta}$, reflecting the convention:
$$
...
$$
    \bT=T^{\alpha}_{\ \beta} \mathbf{e}_{\alpha}\otimes \bomega^{\beta}\quad \text{and}\quad \bT^*=T^{\ \beta}_{\alpha} \bomega^{\alpha}\otimes \mathbf{e}_{\beta}
$$
:::
