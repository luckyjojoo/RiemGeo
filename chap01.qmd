...
$$
where $V^*$ denotes the dual space of $V$. For $\mathbf{v}\in V, \bw\in W$ and $\mathbf{\alpha}\in V^*, \mathbf{\beta}\in W^*$, we define
$$
    \mathbf{v}\otimes \bw(\mathbf{\alpha}, \mathbf{\beta})=\mathbf{v}(\mathbf{\alpha})\bw(\mathbf{\beta})\in \mathbb{R}.
$$
...
## Example
1) Let $V$ be a vector space, and $v\in V$ be a vector. Clearly $\mathbf{v}$ can be viewed as a $(1, 0)$-tensor. In fact we can define $\mathbf{v}(\mathbf{\alpha}):=\mathbf{\alpha}(\mathbf{v})$ for any $\mathbf{\alpha}\in V^*$.
...
To calculate $\mathbf{T}(v)$ for $\mathbf{v}=\mathbf{e}_{\alpha}v^{\alpha}$:
$$
    \mathbf{T}(\mathbf{v})=a_i^{\beta} \delta_{\alpha}^i \mathbf{e}_{\beta} \cdot v^{\alpha} = \mathbf{e}_{\beta}(a_{\alpha}^{\beta} v^{\alpha}) ,
$$
...
$$
For a $(r,s)$ tensor $\mathbf{T}$ and a $(p, q)$ tensor $\mathbf{S}$, for $i=1, 2$, then we define $\mathbf{T}\otimes \mathbf{S}$ by
$$
\begin{aligned}
    (\mathbf{T}\otimes\mathbf{S})&(\alpha_1, \cdots,\alpha_r,\cdots, \alpha_{r+p}, X_1, \cdots, X_s,\cdots, X_{s+q} )\\
    &=\mathbf{T}(\alpha_1, \cdots, \alpha_r, X_1, \cdots, X_s)\mathbf{S}(\alpha_{r+1},\cdots, \alpha_{r+p}, X_{s+1}, \cdots, X_{s+q}),
\end{aligned}
$$
